...
var ding:FlxSprite;
var credits:FlxText;
var cleanupTimer = new FlxTimer();

var path = "songs/" + curSong + "/credits.txt";
...
function postCreate() {
	...
	ding = new FlxSprite(-1280, 150).makeGraphic(1280,300,0xFF000000);
	ding.alpha = 0;

	var cFont = PlayState.SONG.meta.customValues?.creditsFont ?? "sonic2.ttf";
	credits = new FlxText(ding.x, ding.y, ding.width, displayText);
	credits.setFormat(cFont,32, 0xFFFFFFFF, "center", Border.SHADOW, 0xFF000000);
...
function postUpdate() {
	if (running) return;
	cleanupTimer.cancel(); //just in case
	cleanupTimer.start(5, () -> {for (i in [ding, credits]) {
		remove(i);
		i.destroy();
	}});
}